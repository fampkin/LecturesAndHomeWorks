...
class Table:
    ...
    def get_column_types(self, by_number=False):
        mes = set()
        for i, j in self.column_types.items():
            mes.add(self.column_types[i])
            if len(mes) != 1:
    ...
    def get_values(self, column=0):
        try:
            ...
            if col_type == datetime:
                return [datetime.strptime(row[index], "%d-%m-%Y").strftime("%D") for row in self.rows]
    ...
    def get_value(self, column=0):
        try:
            ...
            col_type = self.column_types[index]
            if col_type == datetime:
                return [datetime.strptime(row[index], "%d-%m-%Y").strftime("%D") for row in self.rows][0]
    ...
    def concat(self, another):
        if self.headers != another.headers:
            raise ValueError("Заголовки таблиц не совпадают")
        self.rows.extend(another.rows)
